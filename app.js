...
var logger = log4js.getLogger('PrimeCertWebApp');
...
log4js.configure({
    appenders: [{type: 'console'},
                {type: 'file', filename: 'fabric-api.log', category: 'PrimeCertWebApp'}]
});
app.options('*', cors());
...
logger.debug(' ------>>>>>> 00000new request for %s',req);
...
console.log('********************* Invoking Method 1');
console.log('********************* Invoking Method 2', req.username, req.orgname);
...
// Query Get Block by Transaction ID
app.get('/channels/:channelName/block/:trxnId', async function(req, res) {
	logger.debug('================ GET BLOCK BY TRANSACTION_ID ======================');
	logger.debug('channelName : ' + req.params.channelName);
	let trxnId = req.params.trxnId;
	let peer = req.query.peer;
	if (!trxnId) {
	...
	let message = await query.getBlockByTxId(peer, req.params.channelName, trxnId, req.username, req.orgname);
	res.send(message);
});
...
app.get('/chaincodes/:peer/:channel/:type', async function(req, res) {
	var peer = req.params.peer;
	var installType = req.params.type;
